...
def search_hammingdist(txt, pat, dist):
    m = len(pat)
    n = len(txt)
    segment = round(m/(dist + 1))
    result = [None]*n

    for i in range(dist + 1):
        start = i*segment
        end = min(m, (i+1)*segment)
        matched = boyer_moore(txt, pat[start:end])

        for k in matched:
            if k < start or k-start+m > n:
                continue
            ...
            if count <= dist:
                result[k - start + 1] = count
                #print('result ' + str(k-start+1) + ' ' + str(count))
...
def main():
    ...
    HD = search_hammingdist(txt, pat, 1)
...
if __name__ == "__main__":
    ...
    #search_hammingdist('bbcaefadcabcpqr', 'abc', 1) 
